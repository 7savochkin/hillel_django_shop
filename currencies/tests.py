...
def test_get_currency_task(mocker, faker):
    assert not CurrencyHistory.objects.exists()
    get_currency = mocker.patch('currencies.clients.client.pb_client.get_currency')
    ...
    assert CurrencyHistory.objects.filter(currency='USD', buy='2', sale='3')
